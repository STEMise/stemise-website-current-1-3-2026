...
const contributorCategories = [
  ...
  },
  {
    ...
    benefits: [
      "All General Contributor benefits",
...
const MembershipSection = () => {
  ...
  const [gradientOpacities, setGradientOpacities] = useState<number[]>(
    contributorCategories.map(() => 0)
  );
  ...
  const handleHover = useCallback((index: number | null) => {
    targetOpacitiesRef.current = contributorCategories.map((_, i) => 
      i === index ? 1 : 0
    );
  }, []);
